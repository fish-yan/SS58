...
public class Blake2 {
    ...
    struct blakeConText {
        ...
        init( _ size: Int, _ key: [UInt8], _ salt: [UInt8], _ personal: [UInt8]) {
            self.b = [UInt8](repeating: 0, count: 128)
            self.h = [UInt64](IV[0 ..< 8])
            self.h[0] ^= 0x01010000 ^ (UInt64(key.count) << 8) ^ UInt64(size)
            for i in 0 ..< salt.count {
                if i < 8 {
                    h[4] ^= UInt64(salt[i]) << (i << 3)
                } else {
                    h[5] ^= UInt64(salt[i]) << ((i - 8) << 3)
                }
            }
            for i in 0 ..< personal.count {
                if i < 8 {
                    h[6] ^= UInt64(personal[i]) << (i << 3)
                } else {
                    h[7] ^= UInt64(personal[i]) << ((i - 8) << 3)
                }
            }
            self.t = [0, 0]
    ...
    ///   - key: The key [UInt8], default is an empty array
    ///   - salt: The salt [UInt8], default is an empty array
    ///   - personal: The personalization [UInt8], default is an empty array
    public init(digestLength: Int, key: [UInt8] = [], salt: [UInt8] = [], personal: [UInt8] = []) {
    ...
    public func update(_ data: [UInt8]) {
        for i in 0 ..< data.count {
            if self.ctx.c == 128 {
                self.ctx.t[0] &+= UInt64(self.ctx.c)
    ...
    func rotate(_ x: UInt64, _ n: Int) -> UInt64 {
        return (x >> n) ^ (x << (64 - n))
    }

    func G(_ v: inout [UInt64], _ a: Int, _ b: Int, _ c: Int, _ d: Int, _ x: UInt64, _ y: UInt64) {
        v[a] = v[a] &+ v[b] &+ x
    ...
    func bytes2limb(_ b: [UInt8], _ n: Int) -> UInt64 {
        var x = UInt64(0)
        for i in 0 ..< 8 {
            x |= UInt64(b[i + n]) << (i << 3)
        }
    ...
    func limbs2bytes(_ l: [UInt64], _ n: Int) -> [UInt8] {
        var x = [UInt8](repeating: 0, count: n)
        for i in 0 ..< n {
            x[i] = UInt8(l[i >> 3] >> (8 * (i & 7)) & 0xff)
        }
        return x
    }
